    ...
    <>
      <div
        className={`mt-3 flex items-center justify-between rounded-xl p-3 text-sm ring-1 ${
          isAffordable
            ...
            : 'cursor-pointer bg-red-50 ring-red-200 transition-shadow hover:shadow-md dark:bg-red-900/20 dark:ring-red-800'
        }`}
        {...(!isAffordable ? { onClick: () => setShowUpgrade(true), role: 'button', tabIndex: 0 } : {})}
      >
      ...
      {showUpgrade && (
        <UpgradeModal
          ...
          creditsRemaining={estimate.balance_after != null ? estimate.balance_after + estimate.quoted_credits : undefined}
          creditsRequired={estimate.quoted_credits}
        />
      )}
    </>
  );
